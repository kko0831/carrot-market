...
이번 영상에서는 이 화면을 만드는데 필요한 데이터를 가져옴

상품을 클릭하면 이 화면으로 오게 됨

우리가 해야하는 것은 데이터베이스에서 이 부분을 가져오는 것임

여기는 무엇인지 앎

이름, 가격, 설명임

그리고 유저랑 유사한 상품도 가져와야 함

유사한 상품을 결정하는 기준은 '비슷한 이름을 가졌는가'로 하려 함

예를 들어 이름에 Galaxy라는 단어가 들어가면, 이름에 Galaxy가 들어가는 상품을 골라냄

이런 검색 및 필터링 과정은 Prisma Search API를 이용해서 진행함

가장 먼저 여기 있는 ID를 얻어야 함

우리는 지금 products 페이지 안에 있고 이 파일 이름은 URL에서 변수를 얻어온다는 뜻임

먼저 useRouter로 router를 만들고 router.query를 콘솔로 찍어봄

보다시피 처음에는 아무것도 없다가 그 다음에 id가 들어왔음

이것은 조금 조심해야함

이제 useSWR을 import하고 요청을 보낼건데 url은 /api/products 뒤에 이런 식으로 id가 들어감

하지만 query.id가 있는지 없는지 확실하지 않기 때문에 이렇게는 할 수 없음

이렇게 해도 되기는 함

보다시피 우리가 원하는대로 /api/products/1에 요청을 잘 보내고 있음

하지만 가끔은 이렇게 undefined로 요청을 보내게 됨

이것은 router가 mount 중이기 때문임

그러니까 기본적으로 맨 처음에는 이 id에 접근할 수 없음

그래서 먼저 확인을 해야 함

router.query.id가 있는지 먼저 확인하고, 있으면 요청을 보냄

없으면 null이라고 함

useSWR을 사용할 때 optional query는 이런 식으로 구현함

그런데 여기를 보면 여전히 Not Found 오류가 나옴

이것은 이 URL을 아직 안 만들었으니까 당연함

빠르게 만들어 봄

api 폴더의 products 폴더로 가서, Next.js에서 변수가 들어가는 파일 이름을 만들 때랑 똑같이 만들어 주면 됨

이렇게 만들면 됨

보다시피 api products랑 pages products에서 같은 작명 방식을 사용하고 있음

이것을 복사해서 여기에 붙여 넣음

이 코드는 전부 지움

그리고 이 handler는 GET 요청만 받을 수 있도록 함

이제 res.json으로 응답할건데 ok는 true고, req.query를 콘솔로 찍어봄

프론트엔드에 router.query가 있는 것처럼 백엔드에는 req.query가 있음

새로고침 해보면 여기 나옴

이제 req.query에서 id를 꺼내옴

그리고 해당하는 상품을 찾아야함

이렇게 product 변수를 선언하고, 쿼리의 id와 동일한 id를 갖는 상품을 찾음

이 id는 string이어야 하는데, 보다시피 req.query의 이 id는 string일 수도 있고 string 배열일 수도 있음

그러니 이렇게 toString 함수를 씀

타입스크립트를 쓰는 사람에게만 해당하는 이야기임

id가 string이 아니고 number임

이렇게 앞에 +를 붙여주면 됨

여기에 product를 넣으면 됨

req.query가 string일 수도 string 배열일 수도 있는 이유는 무엇이냐면, 이렇게 route에 id가 여러 개 붙어 있으면 그게 다 여기로 들어가서 그럼

route의 이름이 이렇게 id가 아니라 이런 식의 catch-all route라면 url에 있는 모든 것이 배열 형식으로 들어옴

참고로 이것은 무료 Next.js 강의에 나오는 내용임

그래서 타입스크립트가 오류를 냄

이것을 좀 지움

이 id가 반드시 string이 되도록, 배열일 수도 있는 id를 string으로 바꾼 다음 다시 number로 바꿔줬음

id는 number여야 함

그럼 이제 프론트엔드로 돌아가서 데이터를 콘솔로 찍어봄

잘 나옴

ok는 true고, product도 잘 나오고 있음

이제 진짜 데이터로 바꿔주기만 하면 됨

여기로 와서 제목은 data.product.name이 됨

잘 나옴

가격은 data.product.price임

여기도 마찬가지로 data.product.description임

그런데 새로고침을 하면 문제가 생김

보다시피 data에서 product를 읽으려고 하는데 data가 없음

그러면 이렇게 해야겠지

data나 data가 가진 객체를 쓸 때는 물음표를 써서 존재 유무를 확인하도록 함

data가 있을 때만 name을 사용하고, data가 있을 때만 price를 사용하고, data가 있을 때만 description을 사용할 수 있도록 함

새로고침 해보면 잘 나옴

여기서 중요한 점이 있는데 보다시피 지금 로딩 중인 상태를 처리하지 않고 있음

개인적인 생각으로는 이렇게 하는 것보다 로딩 중이라는 것을 UI에 표시해야 한다고 봄

Loading이라는 글자가 나오게 하거나 제목이 들어가야 하는 자리에 애니메이션이 있는 사각형을 넣거나 함

그렇게 하는 것이 훨씬 낫겠지만 강의 시간을 아끼기 위해서 이렇게 함

하지만 알아둬야 하는 것은 유저가 데이터를 빨리 받아오지 못 하면, 빈 화면을 보게 됨

좋지 않은 일임

여기서 새로고침을 해보면 보다시피 이렇게 됨

유저가 데이터를 빨리 받아오지 못 하면 이것만 보게 됨

그러니 로딩 중인 상태를 직접 처리 해봄

로딩 상태는 기본적으로 data가 있는지 없는지를 확인함

data가 없으면 로딩 중이라는 의미임

아직 유저를 안 가져왔으니까 이제 유저를 가져와야 함

그것은 저번에 해봤지

여기에 include만 넣어주면 됨

그런데 유저의 모든 정보가 필요한게 아니니까 우리가 원하는 것만 찾을 수도 있음

product를 콘솔로 찍어서 한번 봄

여기서 새로고침하고 백엔드로 돌아오면 보다시피 product를 찾아줬음
...
유저 정보 전체가 필요하지는 않고, 지금 같은 경우에는 ID와 이름만 필요함

이렇게 데이터베이스에 선택적으로 요청할 수도 있음

새로고침하고 다시 돌아와 보면, 보다시피 요청한 두 가지만 들어 있음

전부 다 가져오는 것보다 나은 것 같음

createAt이나 updatedAt 같은 것은 필요없음

요청은 적을수록 좋음

avatar도 가져오는게 좋을 것 같음

아직 만들지는 않았지만 나중에 필요함

이제 여기에 data?.product?.user?.name을 쓸 수 있음

그리고 여기 View profile은 a 태그로 바꿈

앵커 태그로 바꾸고 이 두 개를 Next.js Link로 감쌈

그리고 이 Link는 아직 만들지는 않았지만, User Profile 페이지로 연결됨

한번 확인해봄

보다시피 Anonymous라고 잘 나오고, 클릭하면 /users/profiles/9로 잘 이동함

이런 경우에는 id보다 name을 쓰는 것이 나을 것 같음

노마드코더도 id가 아니라 username을 씀

프로필에 username을 사용했음

username을 사용하고 싶다면 model에 새 field를 만들어주면 됨

계정을 만들 때 쓰는 enter form에 새 form도 만들어줌

일단은 이정도면 만족스러움

다음 영상에서는 유사한 상품을 만들거고, 그 다음에는 좋아요와 좋아요 취소 기능을 만듦

## 11.5 Related Products

이번 비디오에서는 Similar Items 항목을 더 진행해보기 전에 이 컴포넌트에 타입스크립트를 조금 추가해줌

그래서 우선 인터페이스를 먼저 만들어줄건데 이름은 ItemDetailResponse라고 해줌

이것은 필드로 ok:boolean을 가지고 그 다음에는 product를 가질건데, Prisma덕분에 Prisma에서 Product 타입을 그냥 import하기만 하면 문제 없음

useSWR에 이 interface를 주면 data가 타입을 가지게 됨

하지만 문제가 있음

기억하다시피 백엔드에서는 product에 user 필드를 포함했었음

하지만 여기에서는 user를 포함하고 있지 않기 때문에 타입스크립트가 잘못됐다고 말하고 있음

타입스크립트에서는 Product의 타입을 보면 user가 없음

그래서 우선 Product를 extends함

인터페이스를 선언하고 이름은 ProductWithUser라고 함

그러면 Product의 모든 필드도 가지고, user라는 User 타입 필드를 하나 더 가짐

이런 식으로 하면 됨

ProductWithUser를 사용하면 문제 없음

이제 data?.product?.user?.name에 빨간 줄이 없음

완벽하게 작동하고 있음

그러면 이제 Similar items로 넘어가봄

Prisma는 아주 좋은 필터 조건들과 operator들을 가지고 있음

예를 들어서 이렇게 원하는 name을 equals를 통해 검색할 수도 있음

좋은 기능임

무엇인가 일치하지 않는다고 하고 싶으면 not을 쓰면 되고, 리스트에 존재하는지를 확인하려면 in을 쓰면 됨

notIn, lt, lte, gt, gte 같은 조건들도 있음

contains도 있음

PostgreSQL을 사용한다면 search라는 옵션도 사용할 수 있음

우리는 아니니까 넘어가고 startsWith나 endsWith도 사용할 수 있음

그리고 그 둘을 AND 또는 OR를 이용해서 같이 묶어서 사용할 수도 있음

여기 보다시피 title에 'Prisma' 또는(OR) 'database'를 포함하고, 거기에다가 (AND) published:false인 포스트를 검색할 수 있음

우리는 아주 많은 OR들을 사용하게 될거고 이렇게 작동하게 됨

유저가 product를 검색하는 handler를 호출할 때, 우리는 제품의 이름을 인자로 받음

제품의 이름이 galaxy s20이나 뭐 그런거라고 해봄

인자로 이런 이름을 받고, 이 이름을 분리해서 배열로 만듦

그리고 map 함수를 사용해서 하나 혹은 여러 개의 OR 안에 들어갈 title 배열들을 만들어줌

여기서는 contains에 'galaxy'와 's40'이 들어가게 됨

그럼 실제로 해봄

우선 여기로 와서 검색하는 거니까 terms이라는 변수를 선언하고, product?.name.split("")으로 배열을 만들어줌

그리고 여기에 공백을 줘서 공백으로 title을 분리함

그러면 공백이 있다면 배열이 만들어짐

여기서 잠깐 빠르게 split 예시를 들어봄

이렇게 'hello/how/are/you'라는 문자열이 있다고 해봄

이것을 이렇게 split("")을 하면 제대로 작동하지 않음

보다시피 이것은 우리가 원하는게 아님

하지만 이렇게 "/"를 기준으로 split하고 싶다고 하면 분리된 것을 알 수 있음

이 경우에는 분리시키는 기준이 공백이 됨

그럼 이제 map 함수를 호출하고 싶음

이제 배열을 가지고 있으니까, 배열의 각 word마다 객체를 반환하고 싶음

화살표 함수에서 객체를 리턴하려면 먼저 소괄호로 이렇게 감싸준 뒤에 객체를 넣어야함

그리고 이 객체가 여기 이 OR 안에 있는 것들처럼 바로 필터링 조건들이 됨

하지만 title 대신에 product에는 name이라는 필드가 있으니까 name을 사용함

그러면 name이라고 적고 그 안에는 contains라고 적는데, 사실 그냥 equalsTo라고 해도 됨

그리고 값은 word가 됨

그러면 console.log(terms)를 해봄

다시 돌아와서 새로고침을 하고 백엔드로 가보면, 결과가 이렇게 나옴

왜냐하면 이것이 정말 우리의 product 이름에 있는 단 하나의 word이기 때문임

prisma studio로 가서, 이 제품의 이름을 galaxy S60이라고 바꿔봄

그리고 save change를 해줌

그럼 이제 product를 찾아봄

그리고 공식문서에 나온 것처럼 where, OR를 적은 다음 우리의 멋진 배열을 넣어주기만 하면 끝임

그럼 이제 relatedProducts를 console.log 해봄

이것이 작동할 거라고 생각한다면 마찬가지로 그렇게 믿고 있음

여기서 보면 우리가 잘 해냈다는 것을 알 수 있음

잘 작동함

보다시피 name에 galaxy와 s20이 포함된 product를 잘 검색했음

하지만 검색 결과에 현재와 똑같은 제품을 또 나오게 하고 싶지는 않음

그럼 무엇을 하면 되냐면 여기에 AND 조건을 추가하고, 현재 제품의 id가 우리가 찾고 있는 제품의 id와 같지 않아야 한다는 조건을 추가함

id를 아까처럼 문자열로 변환해줘야하기 때문에, 타입스크립트가 또 뭐라고 하고 있음

그래서 여기에 cleanID라는 변수를 선언해서 문자열로 먼저 바꿔주는게 나을 것 같음

아니면 그냥 이것을 id: product?.id로 바꾸면 될 것 같음

데이터베이스에서 온 product니까 이것도 작동함

그럼 이제 relatedProducts는 비어있어야 함

다시 확인해보면 아무것도 검색되지 않았음

왜냐하면 검색 결과가 현재 product (s60) 밖에 없었음

그럼 정말 빠르게 상품을 하나만 업로드해봄

여기 와서 클릭하고 이름은 똑같이 Galaxy라고 하고 이번에는 S22가 됨

가격은 12원(!)으로 하고 설명은 'perfect condition'임

업로드까지 완료 했음

그럼 보다시피 Galaxy S22인 상품이 생겼음

아주 잘 작동하고 있음

그리고 다시 콘솔을 확인하면, galaxy S60이 검색 결과로 나왔음

그럼 이제 잘 작동하는 것 같지

다음으로 넘어감

그러면 이제 상품을 보내면서 relatedProducts도 같이 보내줌

relatedProduct에 관한 화면을 만들어서 이 영상을 녹화한 것은 이것을 보여주려고 했던 것임

Prisma를 활용해서 이렇게나 효율적으로 검색할 수 있다는게 매우 멋지다고 생각함

물론 타입스크립트가 자동완성 기능과 함께 여러가지를 알려주기도 함
...
그리고 이제 프론트엔드에서 몇가지 손볼게 있다는 뜻이기도 함

프론트엔드에 있는 여기 이 인터페이스에 relatedProducts 필드를 추가해야함

타입은 Product의 배열이 됨

그 말은 즉 여기에다가는 data?.relatedProducts에 map도 해줘야 한다는 뜻임

여기도 이것 대신 product로 바꿔주고 key는 product.id가 됨

여기는 product.name으로 바꿔주고 $6은 product.price가 됨

그리고 여기에다가는 Link를 줘야할 것 같은데 이것은 숙제로 내줌

여기 이 컨테이너를 감싸는 Link를 넣어서 클릭하면 그 상품 페이지로 갈 수 있도록 해봄

우선은 아주 잘 작동하고 있음

Galaxy S20이 Galaxy S22의 연관 상품이라고 나옴

물론 실제 플랫폼에서 이런 연관 상품 항목은 좀 더 복잡한 로직을 갖고 있음

아마 어떤 머신러닝 알고리즘에 의해 분류를 하거나 검색을 함

이 부분은 단지 핑계거리였음

Prisma가 제공하는 멋진 검색 API로 어떻게 검색할 수 있는지를 보여주고 싶었음

그럼 이제 이 화면은 다 끝났음

다음 영상에서는 좋아요 기능을 만들어 봄

## 11.6 Favorite Products

이번 영상에서는 우리 앱에 Favorite (관심상품) 기능을 추가해봄

이 상품 페이지에서 이렇게 좋아요 버튼을 클릭할 수 있어야겠지

이것을 클릭하면 관심목록에 이 상품이 추가되어야 하고, 다시 클릭하면 관심목록에서 제거돼야 함

그렇게 좋아요를 누른 것들은 여기 이 목록에서 보여야 함

그럼 우선은 favorite을 생성한 다음 favorite 목록을 얻어야겠지

그럼 시작해봄

먼저 해야할 것은 Schema에 model을 추가하는 것임

왜냐하면 관심목록도 데이터베이스 상의 레코드이기 때문임

그럼 favorite 모델을 만들어봄

이름은 Fav라고 하고 id, createdAt, updatedAt을 복사해줌

이제 기억해야하는 것은 favorite을 만드는게 바로 user라는 것임

그러면 필드에 user를 추가해줌

타입은 User가 됨

그리고 favorite은 product에도 연결되어 있음

왜냐하면 유저가 누르는 것은 상품에 대한 좋아요임

그럼 여기에 product 필드를 추가하고 타입도 Product가 됨

그러면 이제 문제 없음

여기서 보이는 것처럼 문제가 있는데, 반대로 User에도 관계를 만들어줘야 함

여기 User 모델에도 favorite 필드를 만들어줌

fav를 가지고 있고, Fav 배열을 넣어줌

Product에 있던 User와의 relation이 똑같으니까, 이것을 그대로 복사해줌

그리고 product 필드에도 relation을 설정해줌

아직 안 만들었지만 fields에는 productId라고 적어줌

product의 ID를 참조하기 위해서 만들어줌

그리고 product를 삭제하면 favorite도 삭제할거니까 onDelete: Cascade도 해줌

두 개의 정수(Int) 필드를 만들건데 하나는 userId고 타입은 Int임
...
왜냐하면 product에도 역관계를 설정해줘야 하기 때문임

product는 여러 개의 fav를 가지게 됨

그래서 favs는 이렇게 Fav 배열을 타입으로 가짐

기억해야 할 것은 우리가 여기서 한 모든 것들은 Fav 레코드라는 것임

유저에 의해서 생성되고 product를 가리킴

그렇기 때문에 fav 데이터를 product에서도 접근할 수 있고, user에서도 접근할 수 있음

그래서 여기 User는 배열 타입의 fav를 가지고, 이것은 fav 목록이 됨

그 말은 곧 유저가 fav 데이터를 백개 생성했다면, User와 Fav에 모두 관계가 있으니까 유저가 몇 개의 fav를 가지고 있는지 셀 수 있을 거라는 얘기임

그리고 product도 마찬가지로 product를 가리키는 모든 fav들을 계속 추적하기 때문에, 여기서 할 수 있는 것은 이 product에 대해 몇명이 favorite으로 추가했는지 쉽게 셀 수 있음

이제 데이터베이스를 업데이트 해볼 차례임

터미널에 npx prisma db push를 입력하면 db를 업데이트 할거고, 여기서 중요한 것은 이 명령을 실행할 때마다 새로운 클라이언트가 생성될 수 있게 next js를 재실행 해줘야 한다는 것을 기억함

아주 중요함

그럼 다음으로 넘어가봄

이제 우리의 Prisma Studio가 작동하는지 한번 봄

새로고침하고 Prisma studio도 재실행 해줌

그럼 이제 여기 Favorite 모델이 생성된게 보임

잘 작동하고 있음

그럼 이제 API 라우팅을 해줄 차례임

무엇을 하려고 하냐면 API에서 이런 url을 갖고 싶음

그래서 이 url로 요청을 보내게 됨

이런 url에 요청을 하면 favorite 데이터를 만들거나 삭제하게 됨

이렇게 하려면 API의 url이 되는 폴더에서 몇가지를 변경해줘야 함

왜냐하면 현재는 url 구조가 products 다음 id로 끝나고 있는데, 원하는 것은 api의 products 뒤에 /fav이 붙는 것임

그 말은 id 부분을 폴더로 바꿔줘야 한다는 뜻임

그리고 그 안에 index.ts를 생성해줌

그러면 API 폴더에서 products/[id]/index.ts 파일은 api/products/[id].ts와 같게 됨

같은 url을 줌

이 주소는 products 폴더에 [id]라는 폴더를 만들던, [id].ts라는 파일을 만들던 똑같음

하지만 여기에 폴더를 만드는 이유는 fav.ts라는 또 다른 파일을 추가하기 위해서임

그럼 [id].ts에 있던 코드를 전부 복사해서 새로운 index.ts에 붙여넣음

[id].ts는 지움

그리고 fav도 handler를 가지게 됨

fav 안에 있는 코드는 필요 없으니까 전부 지움

fav은 POST 요청을 받을 수 있다고 함

그럼 이제 완료했음

여기까지가 두번째 단계였음

첫번째 단계는 model을 만드는 거고, 두번째 단계는 백엔드에 handler를 작성하기 위한 공간을 만들어두는 것임

그래서 파일을 새로 만들었음

이것이 작동하는지 확인하기 위해서 한번 api/products/1로 접속해봄

이 주소로 가면 제품 페이지가 나와야 함

잘 작동함

그러면 이제 api/products/1/fav로 가보면 이 에러는 잘 작동한다는 소리임

왜냐하면 아까 POST 요청만 보낼 수 있게 설정해놨음

아주 잘 작동함

이제 무엇을 하고 싶냐면, 우선 데이터가 이미 db에 존재하는지를 확인하고 싶음

만약 존재한다면 그 데이터를 지워야 할테고, 존재하지 않는다면 데이터를 새롭게 생성함

한번 생각해봄

상품 페이지로 가서 처음으로 좋아요를 누른다고 했을때, db로 가서 유저와 해당 상품에 해당하는 favorite 데이터가 존재하는지를 확인함

없다면 하나를 새로 만듦

하지만 다시 한번 클릭한다면 그건 이것을 favorite에서 제거하고 싶다는 얘기니까, 한번 더 데이터가 있는지 확인해서 있다면 삭제하면 됨

그럼 우선 이미 존재하는 데이터가 있는지 확인하는 것부터 시작해봄

여기서 findUnique를 사용하지 않는 이유는, findUnique에서 사용할 수 있는 조건이 오직 unique 속성만 가능하기 때문임

그리고 우리의 favorite 데이터에서 unique인 것은 id 한개뿐임

이것만 unique함

findUnique는 unique 필드만 쿼리할 수 있기 때문에 findUnique는 쓸 수 없음

그러니 findFirst라고 해줘야 하고 좀 더 넓은 조건으로 검색할 수 있게 됨

예를 들어서 relation 같은 것으로 검색할 수 있음

원한 것이 바로 그거임

여기서 찾으려고 하는 것은 productId가 req.query에 있는 id와 같은 favorite 데이터임

하지만 이것은 숫자니까 이렇게 +를 붙여주고, toString()이라고 해주면 아무 문제 없음

여기 있는 id는 product의 id임

그러니까 id가 1인 제품의 favorite 데이터를 찾고 있음

하지만 거기에 더해서 user도 알아야 함

그럼 이것을 지움

이제 우리는 productId가 query의 id와 같을뿐 아니라, userId가 session의 user.id와 같은 favorite 데이터를 찾음

그래서 이것이 만약에 이미 존재한다면 db에서 지울거고, 존재하지 않는다면 else 안에서 새로 생성해 줌

if 안에서는 삭제해 줌

그럼 우선 삭제하는 것부터 해봄

그래서 만약에 이미 데이터가 존재한다면, 그것은 우리가 데이터를 삭제해줄 거라는 뜻이 됨

그리고 delete는 어떤 성질이 있는데 이렇게 delete function을 사용해서 삭제하려고 하면, unique에 의해서만 삭제할 수 있음

왜냐하면 delete 함수 또한 unique를 갖고 있는 조건으로만 검색함

그래서 이 경우에는 favorite 데이터의 id를 알아야만 삭제할 수 있음

그것이 유일한 unique 필드임

우리의 경우에는 favorite 데이터를 여기서 찾았기 때문에 id를 가지고 있음

그럼 이제 id가 있으니까 그냥 alreadyExists.id를 삭제할 수 있음

잘 작동함

하지만 어떤 데이터를 삭제하려고 하는데 unique 필드가 없다면, 예를 들어 await client.fav.deleteMany라고 해줄 수 있음

여기서는 session의 user.id가 가진 모든 favorite 데이터를 삭제하는게 가능함

하지만 하지는 않음

매우 중요한 부분이니까 그냥 그 차이를 보여주려는 것임

그리고 여기서 자동완성이 도와줄텐데, 그냥 delete라고 사용하면 보다시피 unique 필드가 아니니까 user로는 fav 데이터를 찾을 수 없다고 알려줌

이런 멋진 것이 있다는 얘기였음

다시 한번 favorite 데이터가 이미 존재한다면 삭제해야 한다는 말임

그리고 존재하지 않으면 그것을 생성해야 함

그럼 한번 빠르게 만들어 봄

그리고 data는 첫번째로는 user를 userId로 연결해줌

id는 user의 id임

여기 session의 user 그리고 id, 또 product와도 연결해줘야함

그러면 여기 product라고 해주고 connect 해줌

id는 url에 있는 id가 됨

무엇이 됐든 에러가 없다면 마지막에는 ok: true를 리턴함

다시 복습해보자면 favorite 레코드 모델을 추가하기 위해서 schema를 조금 변경해줬음

데이터가 이미 존재하는지를 검색해서 존재하면 데이터를 삭제할거고, 존재하지 않는다면 생성해줌

기본적으로 이런 일이 일어남

여기서 처음으로 좋아요를 클릭하면 alreadyExists가 false라는 얘기니까 여기서 데이터를 생성함

버튼을 한번 더 클릭하면 alreadyExists가 true라는 얘기니까 데이터를 삭제함

프론트엔드에서 클릭 이벤트를 구현해보고 요청을 보내봄

그리고 이 제품의 좋아요를 클릭하면 버튼이 빨간색으로 바뀔 수 있도록 데이터를 변경해봄

## 11.7 Favorite Products part Two

이번 영상에서는 여기 이것(좋아요)을 클릭해 봄

그러면 DB로 요청이 갈거고, 이 제품이 관심상품인지 아닌지 표시함

Optimistic UI Update라는 것을 사용할건데, 기본적으로 우리가 백엔드에 요청을 보낼때 백엔드의 응답을 기다리지 않고 변경사항을 반영함

예를 들어, 백엔드에 요청을 보내면서 '이봐, 나 기다릴 필요 없지? 어차피 될거니까'이러는 것임

그래서 바로 유저에게 클릭했다는 것을 표시하고 빨간색을 보여줌

모든 것이 정상이라고 백엔드가 응답할 때까지 기다렸다가 빨간색으로 바꿀 필요가 없음

이것이 바로 Optimistic UI Update라는건데, 백엔드로 보낸 요청이 작동할 거라는 것에 낙관적(Optimistic)이라는 뜻임

UI에서 요청이 정상적으로 잘 수행되었다고 가정하고, 그에 따라 UI를 그냥 변경함

여기서 무엇을 할거냐면, onFavoriteClick이라는 함수를 만들어 줌

여기에서 api/products/id/fav로 POST 요청을 보내줌

그럼 여기 좋아요(favorite)를 표시하는 하트 아이콘 버튼에다가 이렇게 써주면 됨

그럼 이제 전에 만들었던 useMutation 함수를 사용해봄

이것은 URL을 필요로 하니까 그냥 이 URL을 그대로 사용할건데, 대신 /fav를 뒤에 붙임

그리고 우리는 멋진 useMutation 함수가 배열을 준다는 것을 알고 있음

배열의 첫번째 변수는 요청을 트리거하는 함수임

이것을 사용할건데 이름은 toggleFav라고 함

onFavClick 안에 toggleFav를 넣으면 끝임

이 함수는 데이터를 필요로 하는데, 있을 수도 있고 없을 수도 있으니까 그냥 빈 객체를 넣어주면 body가 비어있는 post 요청이 됨

보다시피 여기서는 await 같은 것을 사용하고 있지 않음

loading 상태에 따라서 응답을 기다리거나 하는 것도 전혀 하고 있지 않음

그냥 요청을 보내고 나서 모든 것을 변경함

그럼 작동하는지 봄

새로고침 하고 prisma studio를 엶

보다시피 favorite에 아무것도 없음

그럼 이제 여기를 클릭해보면, 백엔드에는 아무 이상 없음

잘 작동하는 것 같음

그럼 다시 prisma studio로 돌아와서 새로고침 해보면, 최소한 하나의 레코드(DB에서 하나의 데이터)가 보여져야 함

id가 1인 유저가, id가 1인 product를 관심상품에 추가했음

아주 잘 작동함

다시 한번 더 클릭해봄

그 말은 id가 1인 유저가 id가 1인 제품을 더 이상 좋아하지 않는다는 얘기임

백엔드에는 에러가 없음

그럼 다시 새로고침하면 여기 있던 레코드가 보이지 않아야 함

mutation이 아주 잘 작동하고 있음

하지만 여기서 버튼을 한번 더 누르게 되면, 이 제품을 favorite으로 가지게 됨

여기까지는 좋음

하지만 새로고침했을 때 이 제품에 좋아요를 눌렀다면, 이것이 빨간색으로 변해있으면 좋겠음

API로 돌아가서 product를 받아올때, 필드 하나를 더 받아와야 한다는 얘기임

여기에 한가지 필드를 더 추가해야 하는데, 그것은 바로 이미 그 제품에 좋아요를 눌렀는지 아닌지를 구분하는 것임

이미 제품에 좋아요를 눌렀는지 안 눌렀는지를 확인해야 함

여기서 isLiked라는 변수를 하나 더 받음

위에서 만듦

조건을 만족하는 첫번째 아이템을 찾을건데 productId가 검색할 product.id와 같고 userId가 user의 id와 같음

여기서 기억할 것은 우리가 user를 요청하고 있지 않다는 사실임

그냥 product랑 연관된 제품들만 요청하고 있음

하지만 이제 user가 필요하니까, 여기에다가 우리가 항상 하는 것을 해줌

query에서 id를 받고, session에서 user를 받음

그럼 userId는 user?.id라고 해줌

그리고 findFirst가 favorite 레코드를 리턴하기 때문에 리턴한 결과를 Boolean으로 감싸줘야 함

그럼 Boolean 안에 넣어줌

그럼 이제 true/false가 되고, 전부 선택하지 않음

만약에 DB를 좀 더 경제적으로 사용하고 싶다면, favorite 레코드에서 모든 필드를 다 가져오지 않도록 여기에서 select로 id만 선택하거나 할 수 있음

그럼 이제 api/products/1로 가면, isLiked가 true라는 것을 볼 수 있음

아주 잘 작동했음

다시 돌아와서 한번 더 클릭해봄

그럼 이제 api로 돌아가서 새로고침 해보면 isLiked가 false임

다시 한번만 더 클릭해서 새로고침 해봄

이러면 isLiked가 true가 되야 하겠지

이 제품에 좋아요를 눌렀지만, 그것을 유저에게 표시해주지 않고 있음

프론트엔드로 가서 tailwind의 능력을 사용하는 것으로 빠르게 수정해봄

cls를 사용하고 안에다가 붙여넣음

그 전에 text-gray랑 hover:text-gray를 잠깐 잘라냄

조건문을 하나 추가함

data?.product?.isLiked가 true면 빨간색으로 표시하고 아니라면 아까 잘라낸 text 속성들을 표시해줌

우선 이것을 복사해서 붙여 넣어줌

text-gray 대신에 text-red라고 해줌

하지만 보다시피 여기 product?.isLiked가 제대로 되고 있지 않음

아직 user가 있는 product에 아직 isLiked 필드가 없기 때문에 타입스크립트가 에러를 냄

이것은 아주 쉽게 고칠 수 있음

그냥 다시 돌아가서 boolean 타입 isLiked를 추가해 주면 됨

그럼 이것은 다 됐고 넘어가도 됨

data?.product?.isLiked가 아니라 그냥 data?.isLiked가 돼야함

여기에 쉼표를 넣어주면 다 됐음

그럼 잘 작동하는지 한번 봄

이것은 우리가 이 제품에 좋아요를 눌렀다는 뜻임

다시 한번 클릭해보면, UI가 변경되고 있지 않음
...
또 다른 아이콘 하나를 찾아볼건데 이렇게 외곽선만 있는 형태 말고, 안이 채워진 솔리드 형태의 아이콘이 필요함

그럼 Hero Icons에서 찾아봄

하트 모양의 솔리드 아이콘을 찾아봄

여기 있는 이 하트 아이콘은 조건문 안에 들어가게 됨

그러면 data?.isLiked가 true면 솔리드 타입의 빨간 하트 SVG를 보여주고, 아니라면 외곽선만 있는 회색 아이콘 SVG를 보여줌

그리고 class는 className으로 고쳐주고, 나머지도 알맞게 바꿔줌

작동하는지 확인해봄

여기로 와서 아직 이 제품에 좋아요를 누르지 않았음

이제 클릭해보면 아직 변경되지 않았는데 이것은 다음 영상에서 다뤄볼거고, 이제 새로고침 해보면 바뀐 것이 보이지

이 제품에 좋아요가 눌러졌음

아주 잘 작동함

배경색상을 500으로 하고 hover는 600으로 바꿔줌

제품 페이지를 보면 이렇게 하트가 있음

정말 거의 다 됐어

다음 영상에서는 Optimistic UI Update를 해봄

이전 영상에서는 백엔드를 했고, 이번 영상에서는 프론트엔드를 했음

이제 다음 영상에서는 useSWR에서 제공하는 mutate라는 아주 멋진 함수를 활용해서 Optimistic UI Update를 하게 됨

## 11.8 Bound Mutations

이번 영상에서는 SWR 캐시에 있는 데이터를 mutate하는 방법을 알아봄

SWR 캐시를 mutate하려는 이유는 전체 UI가 SWR 캐시에 있는 데이터를 표시하고 있기 때문임

예를 들어서 이 하트 모양 버튼을 클릭하고 싶다고 해봄

버튼을 클릭하면 즉시 이 데이터를 변경하고 싶음

API가 응답하기를 기다리거나, API를 다시 fetch하고 싶지 않음

이런 것은 아무 것도 하고 싶지 않음

그냥 그 즉시 유저에게 보여주고, 유저에게 변화를 반영해서 보여주고 싶음

이것이 바로 데이터를 mutate하려는 이유임

두 개의 mutate 함수가 있는데 하나는 여기 있는 이 함수로 bound 함수라고 불리고, 또 다른 하나는 unbound 함수라고 불림

둘 다 각기 쓰임새가 있음

그러니 둘 다 살펴봄

우선 여기 있는 이것을 사용해서 캐시의 데이터를 mutate하는 것이 얼마나 쉬운지부터 봄

아직 API에 요청을 보내고 싶지는 않으니까 이 줄은 주석처리함

그리고 상당히 바보같은 짓을 해볼거기 때문에, useSWR의 타입을 any로 바꿔줌

왜냐하면 에러를 몇 개 일으켜봄

지금 잠시만 바꿔줌

타입스크립트를 사용한다면 잠시동안만 여기에 any를 넣어줌

왜냐하면 타입스크립트는 원래 자체적으로 에러 있는 코드의 실행으로부터 보호해주는데, 지금은 의도적으로 실수를 만들어보고 싶기 때문임

data를 mutate하고 싶다면 무엇을 해야할까

우선 여기서 mutate 함수를 호출함

mutate 함수는 2개의 인자를 받는데 하나는 캐시에 있는 데이터 대신 사용할 새로운 데이터고, 다른 하나는 true 또는 false임

지금은 true라고 해줌

기본값이 true기는 하지만 일단 true라고 적음

2개의 인자가 있다는 것을 알게 됐지

mutate 함수의 이 첫번째 인자에는 원하는 어떤 데이터든 상관없이 넣으면 됨

말 그대로 원하는 어떤 객체든 아무거나 다 됨

hello: 1 이런 것도 상관없음

여기에 적는 것이 어떤 것이든 새로운 데이터가 됨

그리고 물론 이 컴포넌트의 UI는 새로고침 됨

다시 한 번, 여기에는 원하는 것을 아무거나 넣어도 됨

그것이 바로 새로운 데이터가 됨

어떻게 작동하는지 한번 보여주기 위해서, data?.product?.name을 potato라고 해봄

mutate에게 새로운 데이터는 potato라는 이름을 가진 product라고 해봄

다시 한번, 여기 이 부분에는 무엇이든지 넣을 수 있음

무엇을 넣든 간에 그것이 새로운 데이터가 됨

작동하는지 확인해봄

이것을 클릭하면 여기 이 제목이 빠르게 변하는게 보임

그럼 해봄

potato로 바뀐게 보임

보다시피 나머지 데이터는 전부 사라졌음

가격도 사라지고 제품 설명도 사라졌고 유저명도 사라졌음

왜냐하면 말 그대로 데이터를 방금 넣은 이것으로 덮어 씌우고 있기 때문임

현재 데이터를 이 데이터로 바꿈

그러면 여기 API의 응답으로 온 모든 데이터들이 그냥 사라져버림

이것이 바로 우리가 product를 처음 가져오면 SWR이 받는 데이터임

그리고 나서 이 버튼을 클릭하는 그 즉시, 이 데이터를 전부 덮어씌움

그래서 여기를 클릭하면 potato만 나오고 나머지는 전부 사라짐

보다시피 예전 데이터가 사라졌다가 클릭하면 다시 돌아옴

그 이유는 우리가 두번째 인자로 넘겨준 true 때문임

이 두번째 인자는 SWR이 다시 revalidation(재검증)을 하도록 만듦

SWR이 API로 가서 원래 했던 이 요청을 다시 한번 재검증하게 함

이렇게 하는게 맞기는 함

product를 업데이트 하고 싶은거면 다시 백엔드로 가서 product를 가져와야, 가장 최신의 데이터를 가지고 있게 됨

하지만 이 때 유저는 그것을 기다리지 않아도 됨

그래서 이 첫번째 인자는 유저에게 화면UI의 변경사항을 보여주기 위한 부분임

이 두번째 인자는 변경이 일어난 후에 다시 API에서 데이터를 불러올지를 결정하게 해줌

여기서 보여줌

Inspect(검사)를 누르고 네트워크 탭에 들어가서, 버튼을 누르면 보다시피 데이터를 다시 불러오고 있음

한번 더 해봄

클릭하면 매번 새로운 네트워크 통신이 생겨나는 것이 보이지

우리가 데이터를 다시 불러오고 있기 때문임

물론 우리의 유저는 이것을 볼 수 없음

유저는 업데이트한 데이터만을 보게 됨

SWR이 백엔드로 가서 가장 최신 버전의 데이터를 가져옴

원한다면 물론 이 기능을 끌 수도 있음

이렇게 false라고 해주면 됨

이렇게 false라고 하면 아무 일도 일어나지 않음

false일때 클릭하면, potato로 바뀌고 다시 돌아오지 않음

false라고 했기 때문에, SWR이 다시 백엔드로 가서 데이터를 불러오지 않음

아주 간단함

물론 지금은 다시 데이터를 가져왔음

이대로 창을 전환했다가 다시 돌아와도 데이터를 다시 가져옴

여기까지가 바로 차이점임

다시 한 번, 2번째 인자에 true를 넣으면 SWR은 첫번째 인자를 데이터로 갱신함

그것은 곧 바로, 즉시 일어나고 유저도 변화를 즉시 확인하게 됨

하지만 2번째 인자로 true가 들어오면, 모든 UI가 변경된 이후에도 백그라운드에서는 SWR이 이 URL로 가서, 데이터의 최신 버전을 한번 더 불러오게 됨

그러니까 이렇게 생각하면 됨

첫번째 인자에는 가짜 데이터를 넣고, 두번째 인자가 true면 SWR이 다시 '진짜' 데이터를 찾아서 불러옴

여기서는 우리가 유저에게 주는 데이터가 잘못됐기 때문에 사실 제대로 하고 있지는 않음

제품명 "potato"만 주고 있기 때문에 그 외에는 전부 사라져버림

여기서 두번째 인자로 false를 주면 다른 데이터가 전부 사라져버리게 됨

클릭하면 전부 사라짐

원래 있었던 데이터의 모양을 그대로 유지하려면 원하는 것만 업데이트 해야한다는 것을 기억함

이렇게 하기 위해서는 2가지 옵션이 있음

첫번째 옵션은 당연히 타입스크립트를 사용함

타입스크립트를 사용하면 어떻게 되는지 한번 봄

useSWR의 타입을 설정하면 타입스크립트가 에러를 냄

product가 userId나 createdAt 같은 다른 항목도 가져야 한다고 에러를 냄

아주 유용함

이 문제를 고치려면 두 가지를 하면 됨

첫번째는 이렇게 ...data라고 해주면 새로운 데이터 안에 예전 데이터가 전부 들어가게 됨

예전 데이터는 isLiked, relatedProducts, ok 이런 데이터가 됨

그 다음에는 product 안에서 name을 변경할건데 나머지 필드들도 포함함

이렇게 하려면 product 안에서 나머지 필드를 ...data.product 이렇게 가져오고, 그 다음 name만 potato로 변경해줌

여기서 타입스크립트가 에러를 내는데, data가 존재하지 않을 수도 있기 때문임

타입스크립트 에러를 없애기 위해서 if(!data) return; 이라고 해줌

그럼 이제 버튼을 누르면, 나머지 데이터는 그대로 있으면서 이름이 업데이트 되는 것이 보여야 함

작동하는지 봄

화면으로 와서 클릭하면 보다시피 제목만 potato로 바뀌었음

물론 우리의 경우에는 이렇게 이름을 바꾸지는 않음

매우 중요함

우리가 바꿔야 할 것은 바로 isLiked임

이거만 바꾸면 됨

다시 돌아와서 아까처럼 하는 대신에, ...data는 그대로 두고 isLiked만 data.isLiked의 반대로 만들어 주면 됨
...
이제 유저는 API를 기다릴 필요가 없이 아주 빠른 반응형 UI를 보게 됨

toggleFav는 다시 주석을 해제함

백엔드로 요청을 보낼거기 때문에 이것은 계속 사용하지만 다시 데이터를 가져오지는 않을거니까 2번째 revalidate는 false로 놔둠

그냥 캐시만 변경함

revalidate는 false로 놔두면 이제 다 됐음

그럼 이제 아주 잘 작동함

새로고침을 하고 클릭해보면 좋아요가 눌러졌지

보다시피 바로 즉각적으로 변경됨

다시 새로고침 해도 여전히 눌러져있음

이것이 바로 Optimistic UI Update임

요청을 보내고, 요청에 응답이 오기까지 기다리지 않고 그냥 변경함

mutate를 먼저 실행해도 아무 문제 없음

유저에게 먼저 보여준 후에, 백엔드에 요청을 보냄

이것이 바로 제한된(bound) mutate 함수였음

제한된 mutate라는 것은 mutate가 여기 있는 이 데이터만 변경할 수 있다는 뜻임

둘 다 여기있는 하나의 함수로부터 나온 결과 값이니까 이해가 됨

그러니까 이 useSWR의 요청으로부터 받은 데이터를 변경하고 싶다면, 같은 useSWR로부터 나온 mutate 함수를 사용함

하지만 어떤 경우에는 다른 요청의 데이터를 동시에 변경하고 싶을 때가 있음

예를 한번 들어봄

우리가 만들었던 profile 페이지로 가보면 여기서 원한다면 관심목록을 볼 수 있음

좋아요를 누른 제품들임

이 화면에는 아직 가져온 데이터가 없지만 나중에 URL을 가지고 favorite 데이터들을 불러옴

그럼 이것을 좀 더 생각해보면 만약에 좋아요를 다시 클릭해서 좋아요를 해제하면, 그 즉시 관심목록에서 이 제품이 삭제된다면 좋음

그리고 이것은 unbound mutate 함수로 가능함

다른 요청의 데이터를 mutate 할 수 있음

직접 요청한 데이터 뿐만 아니라, 다른 화면에 있는 다른 요청들의 데이터도 변경함

다시, 우리가 만약 unbound mutate 함수를 사용했다면, 당연히 이 요청의 데이터를 변경해서 하트 색깔을 바꿀 수 있음

하지만 거기에 더해서, unbound mutate 함수로는 다른 화면의 데이터를 수정할 수가 있음

그러니까 이런 것도 충분히 가능함

여기를 클릭하면 보냈던 요청의 데이터 뿐만 아니라 프로필 화면에 있는 데이터도 변경할 수 있음

이것이 바로 unbound mutate 함수임

분명 필요할 거라고 생각함

물론 많은 경우에는 그냥 지금 있는 화면의 데이터를 변경하게 되겠지만, 어떤 경우에는 유저가 모르게 다른 화면의 데이터를 변경해야할 경우가 있음

그것은 다음 영상에서 같이 다뤄봄

## 11.9 Unbound Mutations

이번 영상에서는 다른 useSWR hook에서 나온 데이터를 원하는 곳 아무 곳에서나 mutate 할 수 있는 방법을 알아봄

예시를 들어볼건데, 여기 있는 useUser hook을 ItemDetail 페이지에서 사용해보려고 함

이 hook을 사용하면, 로그아웃된 상태라면 /enter 페이지로 리디렉션 된다는 것을 알고 있지

그러면 하고 싶은 것은 하트 버튼을 클릭할 때마다, bound mutate 함수를 사용함

거기에 더해서, ok를 true에서 false로 변경하게 해줄 unbound mutate 함수를 가져옴

처음 화면(/enter)으로 다시 리디렉션 됨

이것은 실제로 앱을 사용할 때는 말이 안 되는 거지만, 원한다면 언제나 어디에서든 자유롭게 모든 컴포넌트들의 데이터를 변경할 수 있다는 것을 보여줌

그럼 이제 unbound mutate 함수를 가져오기 위해서 useSWRConfig라는 hook을 사용함

useSWRConfig는 mutate라는 함수를 줄텐데 보다시피 이전 mutate와 이름이 같아서 충돌이 남

그럼 예전 mutate를 boundMutate로 이름을 바꿔줌

그대로 붙여넣어 주고 unbound mutate는 bound mutate와 살짝 다름

그 이유는 얽매인 것이 없기(unbound) 때문에, 우리가 변경시키려는 데이터를 정확하게 명시해줘야 함

onFavClick 함수에서 mutate를 호출하고 보다시피 첫번째 인자가 data가 아니라 key임

왜냐하면 어떤 데이터를 변경하는지를 알아야 하니까, 이 경우에는 이 키값을 적어줌

말했듯이 이 값은 URL일 뿐만 아니라, 여기 있는 이 데이터의 id가 됨

이것을 붙여넣으면 이 키값을 가진 데이터를 변경하겠다는 뜻임

그리고 변경할 데이터는 data.ok임

그러니까 ok:false라고 해줌

그리고 revalidation을 true나 false를 줄 수 있는데 false라고 함

어떤 경우에는 그 이전 데이터에 접근해야하는 경우가 있잖아

여기 이 isLiked를 변경한 boundMutate에서는 위치가 데이터가 있는 컴포넌트 내부에 있기 때문에, 예전 데이터에 바로 접근할 수 있었음

하지만 어떤 경우에는 컴포넌트 안에 데이터가 없어서 캐시 안에 있는 데이터가 필요할 때가 있음

이 경우를 예를 들어보면, 만약 유저의 데이터를 유저의 기존 데이터를 기반으로 변경하고 싶다면 이 경우에는(unbound mutate) 그것이 불가능함

하지만 bound mutate의 경우에는 필요한 데이터가 컴포넌트 안에 존재하기 때문에 가능함

하지만 다른 컴포넌트의 데이터를 변경할 때는 당연히 그 데이터가 여기 존재하지 않음

좋은 소식은 원한다면 이렇게 함수를 만들 수 있음

이렇게 하면 인자로 기존 데이터를 줌

타입스크립트를 쓴다면 any 타입이라고 해주면 아무 문제 없음

이것을 보여주고 싶었음

왜냐하면 이렇게 ok: false로 로그아웃 시키는 대신에 !prev.ok 이렇게 해줄 수도 있음

예를 들어봄

알려주려는 것은 단지 바꿀 데이터 객체 하나만을 보내는게 아니라, 원한다면 기존에 요청한 데이터를 인자로 주는 함수를 보낼 수도 있다는 걸 알려주고 싶었음

그 다음에 그것을 활용해서 원하는 것을 할 수 있음

bound mutate에서도 똑같이 함수 인자로 기존의 캐시에 있던 데이터를 받을 수도 있음

그러면 data 대신 prev라고 적어주면 이제 다 됐음

타입스크립트라면 prev &&를 붙여주면 그대로 잘 작동함

밑에 있는 mutate는 주석처리 해주고, 클릭해서 boundMutate 함수가 함수를 보내도 잘 작동하는지 한번 봄

클릭하면 보다시피 함수를 보내도 기존에 작동하던 그대로 잘 작동함

아무 문제 없음

그럼 이제 진짜로 user 데이터를 mutate 해봄

브라우저로 가서 하트 버튼을 클릭하면, boundMutate 함수로 isLiked를 변경해야 함

/api/users/me를 키로 가지는 데이터도 변경해야 함

클릭하면 보다시피 잘 작동함

방금 본 것처럼 SWR 캐시의 데이터를 원하는 아무 곳에서나 mutate 할 수 있음

현재 ItemDetail 페이지에 있지만, 우리가 SWR을 사용하고 데이터의 키값을 알고 있기 때문에 이 hook 안에 있는 데이터를 변경할 수가 있음

물론 여기서 우리는 이미 로그인 되어 있는 상태임

새로고침해보면 로그인 되어있음

그냥 표면적으로만 유저를 로그아웃 시킴

당연히 쿠키도 그대로이고, 모든 것이 다 그대로 예전처럼 캐시를 변경해서 리디렉션 시켜주고 있음

이것이 bound mutate와 unbound mutate의 차이였음

보다시피 둘 다 각기 장점이 있고 유용함

만약 해당 화면에서 얻은 데이터만 변경하기를 원한다면, bound mutate를 사용하면 됨

하지만 다른 화면의 데이터를 변경하기를 원한다면 unbound mutate를 사용해야 함

만약 캐시 데이터를 변경하고 싶은 것이 아니라 다시 한번 요청해서 불러오고 싶다면, 키값을 제외하고 나머지 인자들을 전부 지워주면 됨

이렇게 하면 /api/users/me 데이터를 다시 한번 호출하게 되는데 단순하게 refetch함

알아두면 유용함

인자가 없으면 refetch하고, 데이터를 주면 해당 데이터로 mutate함

그리고 마지막 인자가 false이기 때문에, 다시 데이터를 요청하지 않음

unbound mutate는 주석처리한 채로 놔두고 원래대로 돌아감

그럼 다음 섹션에서 봄

상품 페이지는 완료했음

이제 우리는 상품을 추가할 수도 있고, 좋아요를 눌러서 관심목록에 추가할 수도 있음
...
이번 영상에서는 상품에 좋아요 숫자를 추가해줌

다시 한번 상기시켜주고 싶은 것이 있음

이것이 바로 우리의 API 데이터가 생긴 모습임

하지만 이제 여기에다가 한가지를 더 추가할건데, 얼마나 많은 사람들이 이 상품을 관심목록에 추가했는지를 알려줌

이것이 가능하려면 두가지 방법이 있음

첫번째는 Product 모델로 가서, 여기에 원한다면 favCounts 같은 것을 추가해줄 수 있음

이것은 Int 타입이 됨

누구인가 관심목록에 추가한다면 1을 더해줘야 한다는 것을 기억해야 함

그리고 누군가가 favorite에서 삭제한다면 1을 빼줘야 함

이것을 실제로 하지는 않음

왜냐하면 이렇게 해야 한다는 것을 알고 있어야 하는데, 여기를 보면 Product는 이미 어떤 fav가 자신을 가리키고 있는지를 알고 있음

이것은 모델을 생성할 때, 예를 들어 여기서는 fav가 Product를 가리키게 할 때 역으로 product도 자신을 가리키는 fav가 어떤 것인지를 알아야 하기 때문임

user에 대해서도 마찬가지임

예를 들어, 여기 token이 있고 token은 user를 지목하고 있음

그렇기 때문에 user 또한 어떤 token이 자신을 가리키고 있는지를 알고 있음

이러한 원리 때문에 직접 개수를 저장하지 않아도 좋아요가 몇개인지 셀 수가 있음

count를 하는 방법은 아주 간단함

무엇을 하면 되냐면 여기 findMany 안에 include라고 해줌

그럼 여기서 3가지 옵션들이 있는데, favs, users, 그리고 _count임

_count는 relation을 카운트할 수 있게 해줌

그럼 _count라고 해주고 select를 해줄건데, 여기서는 product의 모든 favorites를 세고 싶음

그것이 바로 우리가 여기에서 이 relation이 필요한 이유임

다시 설명함

우리는 지금 products 디렉토리의 index에 있음

url 상으로는 api/products가 됨

여기서 products의 데이터를 받고 있고, favorite 레코드가 product를 지목하고 있기 때문에 그것은 곧 product가 어떤 favorite이 자신을 가리키고 있는지 알고 있다는 뜻임

그리고 필드로 fav의 배열을 가지고 있으니까, 그 개수를 셀 수 있음

그럼 select에는 favs: true라고 적음

여기로 와서 새로고침하면 보다시피 product가 자신이 가지고 있는 favorite의 개수를 가지고 있음

이 경우에는 좋아요를 누른 상품이 이것이 전부이기 때문에, favs 개수가 1개 밖에 없음

보다시피 아주 잘 작동함

여기서는 이렇게 include 하는 것보다 _count를 사용하는 것이 더 나음

include만 하게 되면 전체 favs 레코드를 불러오기 때문에 별로 좋은 생각이 아님

전체 레코드를 다 불러오고 싶은게 아니라, 그냥 favorite의 개수를 세는 것을 원함

그러니까 이렇게 _count에 넣어주면 됨

다시 index.ts로 돌아가서 hearts의 값은 product._count.favs가 됨

이 타입스크립트 타입을 고쳐줘야 하는데, 타입스크립트를 사용중이라면 extends를 사용하면 됨

우리가 전에 products/index에서 했던 것과 같은 방식으로 extends하면 됨

우리가 원하는 필드를 추가하기 위해서 인터페이스를 extends 했었음

이제 새로고침 해보면 숫자 1이 잘 나옴
...
동네생활 부분을 할거지만, 여기서 했던 것을 한번 더 하게 됨

model을 만들고, API handler를 만듦

누군가가 여기 있는 궁금해요를 사용하기 위해서 이것을 클릭하면 초록색으로 색상을 바꿔준다거나 또는 누군가가 여기에다가 질문을 올리면 답변 개수를 실시간으로 업데이트 해주는 것 등을 함

좋은 소식은 우리가 이것을 이미 할 줄 안다는 것임

그리고 여기있는 궁금해요나 답변은 아마 같은 로직을 사용할텐데, favorite 레코드와 똑같은 로직을 사용함

그리고 개수도 똑같은 방법으로 셀거고 그럼 다음 섹션은 우리가 이번 섹션에서 한 것들을 연습해보는게 전부가 됨

라이브 부분도 똑같음

생성하고 SWR로 불러오는 것을 그 전과 똑같이 하면 되니까 쉬움

그러면 동네생활, 라이브 그리고 나의 캐럿 화면을 전부 다 완료하면 그 다음에는 이미지 부분으로 넘어감

그리고 이미지용 클라우드나 이미지 업로드를 어떻게 할건지에 대한 과정 등을 얘기함
